...
def check_crossovers_in_recent_candles(mars_data):
    """
    Check if there was a crossover in the most recent candles.
    Returns a tuple (buy_signal, sell_signal, signal_candle_index)
    """
    if len(mars_data) < 2:
        return False, False, None
    
    current_value = mars_data.iloc[-1]
    previous_value = mars_data.iloc[-2]
    
    buy_signal = False
    sell_signal = False
    signal_candle_index = None
    
    # Buy signal: previously negative, now positive
    if previous_value < 0 and current_value > 0:
        buy_signal = True
        signal_candle_index = len(mars_data) - 1
        
    # Sell signal: previously positive, now negative
